...
// whether secondary weapon wip fired from shootpos is in range of target_world_pos
bool weapon_secondary_world_pos_in_range(vec3d* shootpos, weapon_info* wip, vec3d* target_world_pos);

// return whether shooter (or turret_shooter; may be null) is in range, fov, etc of target_subsys, for multilock
bool weapon_multilock_can_lock_on_subsys(object* shooter, ship_subsys* turret_shooter, object* target, ship_subsys* target_subsys, weapon_info* wip, float* dot = 0);

// return whether shooter (or turret_shooter; may be null) is in fov, etc of a target, for multilock
